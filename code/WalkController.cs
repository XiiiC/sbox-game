...
namespace Template
{
	...
	public class WalkController : BasePlayerController
	{
		...
		public override void Simulate()
		{
			...
			// Fricion is handled before we add in any base velocity. That way, if we are on a conveyor, we don't slow
			// when standing still, relative to the conveyor.
		...
		public virtual void CheckJumpButton()
		{
			...
			if ( Swimming )
			{
				...
				var edgeTrace = Trace.Ray( Pawn.Position, Pawn.Position + Pawn.Rotation.Forward * 64f ).WorldOnly()
					.HitLayer( CollisionLayer.Solid ).Test();
				var closeToTop = Pawn.WaterLevel.Fraction < 0.9f;

				// Higher velocity if we're close to the top and close to the edge of something
				var velMul = edgeTrace && closeToTop ? 3.5f : 1.0f;

				Velocity = Velocity.WithZ( 100f * velMul );
				return;
			}
		...
		public virtual void WaterMove()
		{
			...
			Accelerate( wishdir, wishspeed, 420, Acceleration );
